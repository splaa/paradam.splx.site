...
// paradam.me.loc/UsersController.php
    ...
    class UsersController extends Controller
    {
		...
		{
			$model = new User();
			$this->readValue($model, 'first_name');
			$this->readValue($model, 'last_name');
			$this->readValue($model, 'login');
			$this->readValue($model, 'email');
			
			//TODO: Излишнее поле password достаточно password_hash
			$this->readValue($model, 'password');
			
			$model->setPassword($this->prompt('PasswordHash:', [
				'required' => true,
				'pattern' => '#^.{6,255}$#i',
