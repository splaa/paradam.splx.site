...
class Smsc extends BaseObject
{
	...
	protected $_errorCode = [
		...
		9 => 'Попытка отправки более пяти запросов на получение статуса одного и того же сообщения в течение минуты.Данная ошибка возникает также при попытке отправки более 15 любых запросов одновременно.',
	];

	public function __construct($telephone = '+380980183456', $message = 'code', $config = [])
	{
		parent::__construct($config);
		$this->login = Yii::$app->params['api']['smsc']['login'];
		$this->password = Yii::$app->params['api']['smsc']['password'];
		$this->post = Yii::$app->params['api']['smsc']['post'];
		$this->https = Yii::$app->params['api']['smsc']['https'];
		$this->charset = Yii::$app->params['api']['smsc']['charset'];
		$this->debug = Yii::$app->params['api']['smsc']['debug'];
		$this->from = Yii::$app->params['api']['smsc']['from'];
	 ...
	 * @return bool
	 */
	public function isSuccess($result, $code = false) {
		if (count($result) == 4)
			return true;
		elseif (!empty($send[4]))
			return true;
	...
	{
		$result = $this->send_sms($this->telephone, $this->message);
	...
	{
		return mail("send@send.smsc.ua", "", $this->login.":".$this->password.":$id:$time:$translit,$format,$sender:$phones:$message", "From: ".$this->from."\nContent-Type: text/plain; charset=".$this->charset."\n");
	}
	...
	{
		$url = $_url = ($this->https ? "https" : "http")."://smsc.ua/sys/$cmd.php?login=".urlencode($this->login)."&psw=".urlencode($this->password)."&fmt=1&charset=".$this->charset."&".$arg;
